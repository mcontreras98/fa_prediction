...
def gated_deconv(inputs, skip_input, out_ch, scope='gated_deconv/'):
 #   print(skip_input.get_shape()[3], '========================')
    skip_out = tf.contrib.layers.conv2d(
        skip_input, skip_input.get_shape()[3], conv_size, scope = scope+'conv1', padding='SAME',
        activation_fn=tf.nn.sigmoid, normalizer_fn=None)
    print(skip_out.get_shape(), "=======================11111")
    output = tf.multiply(skip_out, inputs)
    print(output.get_shape(), '=========================2222')
#    output = tf.concat([inputs, skip_input], 3) # use concat or add??
    output = tf.contrib.layers.conv2d_transpose(    
        output, out_ch, deconv_size, scope= scope+'deconv1', stride = 2, padding='SAME',
        activation_fn=prelu, normalizer_fn=tf.contrib.layers.batch_norm, 
        normalizer_params={'scale': True})
...
def generator(down_outputs, z_s, z_r, y, batch_size):
    ...
    print(output.get_shape())
    skip_out = tf.contrib.layers.conv2d(
        down_outputs[0], down_outputs[0].get_shape()[3], conv_size, scope = 'last_conv1', padding='SAME',
        activation_fn=tf.nn.sigmoid, normalizer_fn=None)
    output = tf.multiply(output, skip_out)
