...
class GAN(object):
  ...
  #      self.input_latent_r = tf.placeholder(tf.float32,[None, self.conf.hidden_size])
      ...
      #  self.iter_number = tf.placeholder(tf.int32)
        ...
        # trial_img = np.zeros((self.conf.height, self.conf.height, 3))
        # trial_img[:,:,1] = self.X_rec_s
        # imsave(os.path.join(self.conf.working_directory, 'trial_img.png'),
        #        trial_img)
    #    print(self.X_rec.get_shape())
        ...
        #with tf.variable_scope('Generator') as scope:
            #self.l2_loss = self.get_l2_loss(self.var_gen)
        ...
        #self.gen_loss= self.l2_loss + self.g_loss*self.conf.gamma_gen
   ...
   #     return tf.reduce_mean(-target_tensor * tf.log(output_tensor + epsilon) -(1.0 - target_tensor) * tf.log(1.0 - output_tensor + epsilon))
    ...
    def save_image(self, imgs, imgs2, inputs, epoch):
        ...
        for k in range(self.conf.batch_size):
            ...
            res[:,0:self.conf.height,:]= inputs[k,:,:,:] * 255
            res[:,self.conf.height+2:self.conf.height*2+2,0]=inputs[k,:,:,0] * 255
            res[:,self.conf.height+2:self.conf.height*2+2,2]=inputs[k,:,:,2] * 255
            res[:,self.conf.height*2+4:self.conf.height*3+4, 1]= inputs[k,:,:,1] * 255
            res[:,self.conf.height*3+6:self.conf.height*4+6, 1]= imgs[k,:,:,0] * 255
            #res[:,self.conf.height*3+6:self.conf.height*4+6, 1] = res[:,self.conf.height*3+6:self.conf.height*4+6, 1] * 255
            # res[:,:,1] = res[:,:,1] * 255
            res[:,self.conf.height*4+8:self.conf.height*5+8, 0]= inputs[k,:,:,0] * 255
            res[:,self.conf.height*4+8:self.conf.height*5+8, 2]= inputs[k,:,:,2] * 255
            res[:,self.conf.height*4+8:self.conf.height*5+8, 1]= imgs[k,:,:,0] * 255
            #res[:,self.conf.height*4+8:self.conf.height*5+8, 1] = res[:,self.conf.height*4+8:self.conf.height*5+8, 1] * 255
            res[:,self.conf.height*5+10:self.conf.height*6+10, 1]= imgs2[k,:,:,0] * 255
            #res[:,self.conf.height*5+10:self.conf.height*6+10, 1] = res[:,self.conf.height*5+10:self.conf.height*6+10, 1] * 255
        ...
        for i in pbar(range(self.conf.max_test_epoch)):
            ...
            for j in range (self.conf.max_generated_imgs):
                ...
                for k in range(output_test.shape[0]):
                    ...
                    print("k is {}".format(k))
                    print("y label is {}".format(y_label[k]))
                    ...
                    #imsave(os.path.join(imgs_original_folder,'epoch_%d_#img_%d_cls_%d.png') %(i,k,y_label[k]),
                        #x[k,:,:,:] * 255)
                    imsave(os.path.join(temp_test_dir, 'ground_truth.png'),
                           x[k, :, :, :] * 255)
                    res = np.zeros((self.conf.height, self.conf.height, 3))
                    #res[:,:,(0, 2)] = x_extracted[k,:,:,:]
                    res[:,:,1] = output_test[k,:,:,0] * 255
                    #res[:,:,1] = res[:,:,1] * 255
                    print(res.shape)
                    #imsave(os.path.join(temp_test_dir,'imgs_%d.png') %j,
                        #res)
                    imsave(os.path.join(temp_test_dir, 'prediction.png'),
                           res)
                    res2 = np.zeros((self.conf.height, self.conf.height * 5 + 8, 3))
                    res2[:, 0:self.conf.height, :] = x[k, :, :, :] * 255
                    res2[:, self.conf.height + 2:self.conf.height * 2 + 2, 0] = x[k, :, :, 0] * 255
                    res2[:, self.conf.height + 2:self.conf.height * 2 + 2, 2] = x[k, :, :, 2] * 255
                    res2[:, self.conf.height * 2 + 4:self.conf.height * 3 + 4, 1] = x[k, :, :, 1] * 255
                    res2[:, self.conf.height * 3 + 6:self.conf.height * 4 + 6, 1] = output_test[k, :, :, 0] * 255
                    # res[:,self.conf.height*3+6:self.conf.height*4+6, 1] = res[:,self.conf.height*3+6:self.conf.height*4+6, 1] * 255
                    # res[:,:,1] = res[:,:,1] * 255
                    res2[:, self.conf.height * 4 + 8:self.conf.height * 5 + 8, 0] = x[k, :, :, 0] * 255
                    res2[:, self.conf.height * 4 + 8:self.conf.height * 5 + 8, 2] = x[k, :, :, 2] * 255
                    res2[:, self.conf.height * 4 + 8:self.conf.height * 5 + 8, 1] = output_test[k, :, :, 0] * 255
                    # res[:,self.conf.height*4+8:self.conf.height*5+8, 1] = res[:,self.conf.height*4+8:self.conf.height*5+8, 1] * 255
                    #res2[:, self.conf.height * 5 + 10:self.conf.height * 6 + 10, 1] = imgs2[k, :, :, 0] * 255
                    # res[:,self.conf.height*5+10:self.conf.height*6+10, 1] = res[:,self.conf.height*5+10:self.conf.height*6+10, 1] * 255
                    imsave(os.path.join(temp_test_dir, 'target_error_panel.png'), res2)
